...
-- =============================================
-- STORED PROCEDURE: Lấy TẤT CẢ Rooms (để load vào memory khi server khởi chạy)
-- =============================================
IF EXISTS (SELECT * FROM sys.procedures WHERE name = 'SP_GET_ALL_ROOMS')
    DROP PROCEDURE SP_GET_ALL_ROOMS;
GO
...
BEGIN
    ...
    -- Lấy tất cả rooms KHÔNG phải EMPTY và FINISHED
    -- Để server load vào memory khi khởi chạy
    SELECT 
        ROOM_ID AS RoomId,
        ROOM_CODE AS RoomCode,
        ROOM_NAME AS RoomName,
        ...
        LAST_ACTIVITY AS LastActivity
    FROM ROOMS
    WHERE ROOM_STATUS NOT IN ('EMPTY', 'FINISHED')
    ORDER BY CREATED_AT DESC;
END
GO

PRINT '✅ SP_GET_ALL_ROOMS created';
GO

-- =============================================
-- STORED PROCEDURE: Xóa Room khỏi database
-- =============================================
IF EXISTS (SELECT * FROM sys.procedures WHERE name = 'SP_DELETE_ROOM')
    DROP PROCEDURE SP_DELETE_ROOM;
GO
...
BEGIN
    ...
    -- Xóa các matches liên quan trước (nếu có)
    DELETE FROM MATCHES 
    WHERE ROOM_ID = (SELECT ROOM_ID FROM ROOMS WHERE ROOM_CODE = @RoomCode);
    
    -- Xóa room
    DELETE FROM ROOMS WHERE ROOM_CODE = @RoomCode;
    
    SELECT 1 AS Success, N'Room deleted successfully' AS Message;
END
GO

PRINT '✅ SP_DELETE_ROOM created';
GO
